...
function mainController($scope, $http, $location, $window, $uibModal, $log, $rootScope, FileUploader, focus) {
		...
		$scope.createNewElement = function (elementType){
			...
			data: {"path":$scope.path,"name":$scope.newElementName}})
				...
				function errorCallback(response) {
					//console.log(response)
					if (response.status==400) {toastr["error"]("名称已存在", "错误"); $scope.blockButtons=false; $scope.blockButtonsClass='';return;}
		...
		$scope.deleteElement = function (elementName,thatis,hide){
			...
			//Delete folder//START
			if (thatis == 'Folder'){
				if (tempVal) if (!confirm('您确定要删除这个文件夹吗： ' + elementName)) return;
			...
			//Delete file//START
			if (thatis == 'File'){
				if (tempVal) if (!confirm('你确定要删除这个实验室吗： ' + elementName)) return;
		...
		$scope.deleteALLElement = function (){
			...
			if (ObjectLength(folderArray) == 0 && ObjectLength(fileArray)==0){toastr["warning"]("请选择要删除的项目", "警告"); return;}
			var folderCount=1;
			var fileCount=1;
			var tempAllNames='';
			...
			if (confirm('你确定要删除这个项目吗： ' + tempAllNames+'?')){
				for (var foldername in folderArray){
		...
		$scope.editElementShow = function(){
			...
			if (trueCheckbox == 0) {toastr["warning"]("请选择要重命名的项目", "警告"); return;}
			if (trueCheckbox > 1) {toastr["warning"]("您可以重命名一个项目", "警告"); return;}
		...
		$scope.exportFiles = function(){
			$(".content-wrapper").append("<div id='progress-loader'><label style='float:left'>建立档案中...</label><div class='loader'></div></div>")
			...
			if (ObjectLength(fileExportArray) == 0){toastr["warning"]("请选择要导出的项目", "警告"); return;}
		...
		$scope.moveto = function(){
			...
			if (ObjectLength($scope.folderArrayToMove) == 0 && ObjectLength($scope.fileArrayToMove)==0){toastr["warning"]("请选择要移动的项目", "警告"); return;}
