...
$(document).on('click', '.action-networkdelete', function (e) {
    ...
    var body = '<div class="form-group">' +
                    '<div class="question">你确定要删除这个设备吗？</div>' +
                '</div>' +
                '<div class="form-group">' +
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="networkdelete" class="btn btn-success"  data-path="'+id+'" data-dismiss="modal">' + MESSAGES[209] + '</button>' +
...
// <form id="form-picture-delete" data-path="' + picture_id + '" class="form-horizontal form-picture" novalidate="novalidate">
    ...
    if($(this).hasClass('action-nodedelete')) {
        textQuestion = '您确定要删除这个设备吗?'
    } else {
        textQuestion = '您确定要删除选定的设备吗?';
    }

    var body = '<div class="form-group">' +
                    '<div class="question">'+textQuestion+'</div>' +
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="deteleNode" class="btn btn-success" data-path="'+id+'" data-dismiss="modal">' + MESSAGES[209] + '</button>' +
                        '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' +
                    '</div>' +
                '</div>'
    var title = "警告"
...
$(document).on('click', '.delete-picture', function (ev) {
    ...
    var body = '<div class="form-group">' +
                    ...
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="formPictureDelete" class="btn btn-success"  data-path="'+id+'" data-dismiss="modal">' + MESSAGES[209] + '</button>' +
                        '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' +
                    '</div>' +
                '</div>'
    var title = "警告"
...
$(document).on('click', '.action-selecteddelete', function (ev) {
    ...
    var body = '<div class="form-group">' +
                    ...
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="selectedDelete" class="btn btn-success"  data-path="'+id+'" data-dismiss="modal">' + MESSAGES[209] + '</button>' +
                        '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' +
                    '</div>' +
                '</div>'
    var title = "警告"
...
$(document).on('click', '.action-nodesbootdelete, .action-nodesbootdelete-group', function (ev) {
    ...
    var textQuestion = '您确定要删除所有设备的开机启动配置吗?';
    if(self.hasClass('action-nodesbootdelete-group')){
        textQuestion = '您确定要删除选中设备的开机启动配置吗?';
    }
    var body = '<div class="form-group">' +
                    '<div class="question">' + textQuestion + '</div>' +
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="nodesbootdelete" class="btn btn-success"  data-dismiss="modal">' + MESSAGES[209] + '</button>' +
                        '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' +
                    '</div>' +
                '</div>'
    var title = "警告"
...
$(document).on('click', '.action-nodewipe, .action-nodeswipe, .action-nodewipe-group', function (e) {
    ...
    if(self.hasClass('action-nodewipe')){
        textQuestion = '你确定要擦拭这个设备吗?'
    } else if(self.hasClass('action-nodeswipe')){
        textQuestion = '你确定要擦拭所有设备吗?'
    } else {
        textQuestion = '您确定要擦拭选中的设备?'
    }

    var body = '<div class="form-group">' +
                    '<div class="question">' + textQuestion + '</div>' +
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="node_wipe" class="btn btn-success"  data-dismiss="modal">' + MESSAGES[209] + '</button>' +
                        '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' +
                    '</div>' +
                '</div>'
    var title = "警告"
...
$('body').on('click', '.action-textobjectdelete', function (ev) {
    ...
    var id = $(this).attr('data-path')
    var self = $(this);
    var textQuestion = $(this).hasClass('customText') ? '你确定要删除这段文字吗?'
                                                      : '你确定要删除这个图形吗?'
    var body = '<div class="form-group">' +
                    '<div class="question">'+ textQuestion +'</div>' +
                    '<div class="col-md-5 col-md-offset-3">' +
                        '<button id="textobjectdelete" class="btn btn-success"  data-path="'+id+'" data-dismiss="modal">' + MESSAGES[209] + '</button>' +
                        '<button type="button" class="btn" data-dismiss="modal">' + MESSAGES[18] + '</button>' +
                    '</div>' +
                '</div>'
    var title = "警告"
